...
try {
		...
		const ownerNumberr = ["6285793887010@s.whatsapp.net",`${NomorOwner}@s.whatsapp.net`]
		...
		// const jepo = reply('Jepo baitu jepo')
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Faizz_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `${NamaBot}`, "footerText": "*_© Faizz_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online😎🙏`, "*_© Faizz_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: thubb, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:thubb,sourceUrl:`https://wa.me/62817766771250?text=F`}}})
		}
		const simir = (teks) => {
...
if (stdout) {
denz.sendMessage(from, stdout, text, term)
}
})
}
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
			...
			/* if (bad.includes(messagesD)) {
				denz.sendMessage(ownerNumber, 'Toxic Detected!\n${jmn}', text)
				}
				*/
				// troli gua lock untuk selfbot biar kaga kacauu
...
denz.updatePresence(from, Presence.recording)
   ...
   const sotoy = [
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
        ...
        case 'menu':
        case 'help':
        ptod = "6285793887010@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
       ...
       menu = `${uwu}
╭─❒ Menu ❒
│◦➛Creator SC : @${dtod.split('@')[0]}
│◦➛Recode : Faizz
│◦➛Battery : ${baterai.battery}
│◦➛Mode : ${publik ? 'Public' : 'Self'}
│◦➛Total Hit : ${cmhit.length}
│◦➛Prefix : ${multi ? 'Multi Pref' : 'No Pref'}
│◦➛Nama Saya : Faizz
...
Self bot tidak akan merespon ke orang lain.
...
└─────────────────❒
${uwu}`
sendButLocation(from, `${menu}`, `${uwu}Faizz右${uwu}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}command`,buttonText:{displayText:'MENU LIST✍️'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'DEVELOPER👤'},type:1},{buttonId:`fngen`,buttonText:{displayText:'👑'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
break
case 'command':
 listMsg = {
...
client.browserDescription = ['MyFaiz jadibot','Desktop','3.0']
...
sendMess(memekkkk, 'Wkwkwk', MessageType.text)
break
				case 'script':
		case 'sc':
		case 'sourcecode':
		denz.sendMessage(from, { text: "https://github.com/dcode-denpa/bitch-boot\n\nhttps://github.com/fzxcodes", matchedText: 'https://github.com/dcode-denpa/bitch-boot', description: "", title: "Thanks Denis! 😎👌", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
              ...
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                ...
                let exc = budo.replace(sep[0]+"\n", "")
                exec(exc, (err, stdout) => {
					if (err) return denz.sendMessage(from, `root @Faizz-Botz:~ ${err}`, text, { quoted: mek })
					if (stdout) {
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    denz.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`Fake Location`}, MessageType.location)
...
break
            ...
            break
				case 'attp':
					...
					atetepe = await getBuffer(`https://api.xteam.xyz/attp?file&text=${c}`)
...
Giliran = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Faizz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://youtube.com/c/YuzzuKamiyaka"}}})
	}
    }
    }
    }
